-- Managed group invites foundation required before account invite claims.
...
drop policy if exists "Members can read invite usage for their groups" on public.group_invite_uses;
drop policy if exists "Authenticated users can record their invite usage" on public.group_invite_uses;
